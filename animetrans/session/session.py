...
class TrainSession:
    ...
    def tb_train_log(self, global_step, metrics: Mapping[str, Union[float, Image.Image]]):
        for logger in self.loggers:
            logger.train_log(global_step, metrics)

    def tb_eval_log(self, global_step, model: TimmModel, metrics: Mapping[str, Union[float, Image.Image]]):
        for logger in self.loggers:
            logger.eval_log(global_step, model, metrics)
